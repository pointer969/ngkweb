        ...
        .find({}, function(err, wqc){
          pwqc.count().exec(function(err, count){
              if (count > 0) {
                    ...
                    { title: 'NGK | Water Quality Control', 
                        list: wqc,
                        user_info: req.user,
                        baseuri: baseurl,
                        page: page + 1,
                     ...
                     res.render('watercontrol/new', { title: 'NGK | New Water QC Input',baseuri:baseurl});
                  }
            });        
        })  
        .sort({$natural:-1})      
...
pwqcController.create = function(req, res){         
    ...
    res.render('watercontrol/new.jade', { title: 'NGK | New Water QC Input',baseuri:baseurl});
 }   
 ...
 if (req.params.id != null || req.params.id != undefined) {      
  pwqc.findOne({_id: req.params.id}).exec(function (err, wqc) {
        if (err) {         
          ...
          res.render('watercontrol/show', { title: 'NGK | New Water QC Input', wqclist: wqc, baseuri:baseurl});
        }
      });
  } else {    
...
pwqcController.edit = function(req, res){ 
  ...
  pwqc.findOne({_id: req.params.id}).exec(function (err, upwqc) {
        if (err) {
          ...
          res.render('watercontrol/edit', { title: 'NGK | New Water QC Input', wqclist: upwqc, baseuri:baseurl});
        }
      })
 }
...
pwqcController.update = function(req, res){  
   ...
   function (err, profile) {                                                              
        if (err) {         
          ...
          res.render("watercontrol/edit", { title: 'NGK | New Water QC Input',wqclist: req.body, baseuri:baseurl})
        }else{          
