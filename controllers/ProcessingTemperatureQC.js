      ...
      twqc.count().exec(function (err, count) {
        if (count > 0) {
          res.render('tempcontrol/index',
            {
              title: 'NGK | Temperature Quality Control',
              list: wqc,   
              ptype: pid,    
              pdesc: pdescription,       
              user_info: req.user,
          ...
          // res.render('tempcontrol/new', { title: 'NGK | New Temperature QC Input', baseuri: baseurl,places: place });
            ...
            if (!err) {
              ExtensiveValue.findOne({ class: exclass._id, active: true })          
                .exec(function (err, values) {
                  if (!err) {
                    res.render('tempcontrol/new.jade', { title: 'NGK | New Temperature QC Input', baseuri: baseurl, places: values, pdesc:pdescription, ptype:pid });
                  } else {
    ...
    ExtensiveClass.findOne({ class: pid, active: true })
     .exec(function (err, exclass) {
      if (!err) {
        ...
        req.flash('alert-danger', "Error on Load Classes:" + err)
        res.render('tempcontrol/new.jade', { title: 'NGK | New Temperature QC Input', baseuri: baseurl, places: '', pdesc:pdescription, ptype:pid });      
      }
    })
 }
  ...
  if (req.params.id != null || req.params.id != undefined) {
    twqc.findOne({ _id: req.params.id }).exec(function (err, tqc) {
      if (err) {
        ...
        res.render('tempcontrol/show', { title: 'NGK | New Temperature QC Input', tqclist: tqc, baseuri: baseurl });
      }
    });
  } else {
...
twqcController.edit = function (req, res) {
  ...
  twqc.findOne({ _id: req.params.id }).exec(function (err, uptqc) {
    if (err) {
      ...
      res.render('tempcontrol/edit', { title: 'NGK | New Temperature QC Input', tqclist: uptqc, baseuri: baseurl });
    }
  })
 }
  ...
  twqc.findByIdAndUpdate(
    ...
    function (err, profile) {
      if (err) {
        ...
        res.render("tempcontrol/edit", { title: 'NGK | New Temperature QC Input', tqclist: req.body, baseuri: baseurl })
      } else {
...
twqcController.save = function (req, res) {
  ...
  wqc.save(function (err) {
    ...
    res.render('tempcontrol/new.jade', { title: 'NGK | New Temperature QC Input', baseuri: baseurl });
  })
 }
