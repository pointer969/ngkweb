    ...
    .find({}, function (err, documents) {
      Delivery.count().exec(function (err, count) {
        if (count > 0) {
          ...
          res.render('documents/new.jade', { title: 'NGK | New File', baseuri: baseurl });
        }
      });
    })
    .limit(limit)
...
exports.confirmation = function (req, res) {
  var baseurl = req.protocol + "://" + req.get('host') + "/"
  res.render('delivery/confirmation', { title: 'NGK | New Confirmation', baseuri: baseurl });
};
...
exports.save = function (req, res) {
  ...
  upload(req, res, (error) => {
    if (error) {
      ...
      res.render('documents/new', { title: 'NGK | New File', baseuri: baseurl })
    } else {
