        ...
        .find({}, function(err, Lots){
          Lot.count().exec(function(err, count){
              if (count > 0) {
                    ...
                    { title: 'NGK | Receive by TAG (Lakes)', 
                        list: Lots,
                        user_info: req.user,
                        baseuri: baseurl,
                        page: page + 1,
                    ...
                    res.render('lots/new.jade', { title: 'NGK | Receive by Manual (Lakes)',baseuri:baseurl});
                  }     
            });        
        })        
        .limit(limit)
...
lotController.create = function(req, res){         
    var baseurl = req.protocol + "://" + req.get('host') + "/"     
    res.render('lots/new.jade', { title: 'NGK | Lot creation',baseuri:baseurl});
 }   
...
lotController.save  =   function(req, res){
    ...
    lot.save(function(err) {
      ...
      res.render('lots/manual.jade', { title: 'NGK | New Lot', baseuri:baseurl});
    })
 }
                  ...
                  if(!err){
                    Lot.count().exec(function(err, count){
                      var cnt = count+1;
                        ...
                        { title: 'NGK | Receive by Manual (Lakes)',             
                            user_info: req.user,
                            baseuri: baseurl,
                            products: product,
                            drivers: driver,
...
lotController.offflavor = function(req, res) {   
    var baseurl = req.protocol + "://" + req.get('host') + "/" 
        ...
        { title: 'NGK | Off-Flavor Quality Control (Lakes)',             
            user_info: req.user,
            baseuri: baseurl
          });
...
lotController.goodreceiveconfirmation = function(req, res)  {
    ...
    GR.save(function(err) {
      ...
      res.render('lots/goodreceive.jade', { title: 'NGK | Confirmation', baseuri:baseurl});
    })
}
