              ...
              .find({}, function (err, Lots) {
                LotTracking.count().exec(function (err, count) {
                  if (count > 0) {
                    ...
                    res.render('lots/newposition.jade', { title: 'NGK | Add Position', baseuri: baseurl, suppliers: suppliers, vehicles: vehicles });
                  }
                });
              })
              .limit(limit)
  ...
  Suppliers
    .find({}, function (err, suppliers) {
      if (!err) {
        Vehicle.find({}, function (err, vehicles) {
          if (!err) {
            res.render('lots/newposition', { title: 'NGK | Add Position', baseuri: baseurl, suppliers: suppliers, vehicles: vehicles });
          }
        })
      }
    })
